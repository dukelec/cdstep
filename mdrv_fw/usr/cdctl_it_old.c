...
static uint8_t version = 0x00;
static bool _clr_flag = false; // need manual clr flag if version < 0x0e
        ...
        spi_t *spi, gpio_t *int_n, irq_t int_irq)
{
    ...
    version = cdctl_reg_r(dev, CDREG_VERSION);
    dn_info(dev->name, "chip version: %02x\n", version);
    if (version < 0x0a || version > 0x10) {
        dn_error(dev->name, "version error\n");
        return -1;
    }
    _clr_flag = version >= 0x0e ? false : true;
    ...
    if (dev->state == CDCTL_RD_FLAG) {
        ...
        if (val & (CDBIT_FLAG_RX_LOST | CDBIT_FLAG_RX_ERROR | CDBIT_FLAG_RX_BREAK)) {
            ...
            if (_clr_flag) {
                ...
                cdctl_reg_w_it(dev, CDREG_RX_CTRL, (1 << 2) | (1 << 3) | (1 << 5));
                return;
            }
        }
        if (val & (CDBIT_FLAG_TX_CD | CDBIT_FLAG_TX_ERROR)) {
            ...
            if (_clr_flag) {
                dev->state = CDCTL_REG_W;
                cdctl_reg_w_it(dev, CDREG_TX_CTRL, (1 << 2) | (1 << 3));
                return;
            }
        }
